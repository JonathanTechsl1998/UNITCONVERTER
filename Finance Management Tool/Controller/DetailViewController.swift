...
class DetailViewController: UIViewController,  NSFetchedResultsControllerDelegate, UITableViewDelegate, UITableViewDataSource, UIPopoverPresentationControllerDelegate{
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if let category = selectedCategory{
            ...
            if category.categoryExpense!.count == 0 {
    ...
    func updateSpentAmount() {
        let objects = self.selectedCategory?.categoryExpense?.allObjects as! [Expense]
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "editExpense" {
            if let indexPath = expenseTableView.indexPathForSelectedRow{
                let object = (self.selectedCategory?.categoryExpense?.allObjects as! [Expense])[indexPath.row]
                let controller = (segue.destination as! UINavigationController).topViewController as! AddEditExpenseViewController
                controller.addExpenseDelegate = self
    ...
    // MARK: - Configuring the Table View cell
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        print("selected \(self.selectedCategory?.categoryExpense?.count)")
        let sectionInfo = (category.categoryExpense?.allObjects as! [Expense])
        if selectedCategory == nil{
            categoryDetailView.isHidden = true
        ...
        let count  = (category.categoryExpense?.allObjects as! [Expense]).count
        return count
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let expense = (self.selectedCategory?.categoryExpense?.allObjects as! [Expense])[indexPath.row]
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete{
            ...
            let deleteExpense = (self.selectedCategory?.categoryExpense?.allObjects as! [Expense])[indexPath.row]
...
extension DetailViewController: AddEditExpenseViewControllerDelegate {
    func completeSave() {
        self.save()
    }
