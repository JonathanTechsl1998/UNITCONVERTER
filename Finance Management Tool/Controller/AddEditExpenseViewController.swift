...
class AddEditExpenseViewController: UITableViewController, UIPopoverPresentationControllerDelegate, UITextViewDelegate {
    ...
    var addExpenseDelegate: AddEditExpenseViewControllerDelegate?
    ...
    func configureView() {
        ...
        if let category = selectedCategory{
            let objects = category.categoryExpense?.allObjects as! [Expense]
        ...
        if let expense = editingExpense{
            ...
            if let expenseOccurrence = occurrenceSegmentController{
                ...
                self.selectedOccurrence = expense.occurance
                expenseOccurrence.selectedSegmentIndex = Int(expense.occurance)
            }
        }
        ...
        if self.calendarReminder {
            ...
            if permissionGrantedCalendar {
                ...
                do{
                    ...
                    print("Event.eventIdentifier \(String(describing: event.eventIdentifier))")
    ...
    func deleteEvent(){
        if self.eventstoreEventId != "" {
            let event = eventStore.event(withIdentifier: self.eventstoreEventId)!
        self.calendarReminder = false
    ...
    @IBAction func handleAddButton(_ sender: UIBarButtonItem) {
        if validateInputFields(){
            ...
            expense.setValue(self.selectedOccurrence, forKey: "occurance")
            ...
            selectedCategory?.addToCategoryExpense(expense as! Expense)
...
extension AddEditExpenseViewController: AddEditExpenseViewControllerDelegate{
    func completeSave() {}
}
